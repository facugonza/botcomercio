...
const flowPrincipalComercio = addKeyword(
  "flowPrincipalTelefonoNoAsociadoComercio",
  { sensitive: false }
)
  .addAnswer(
    [
    ...
    async (ctx, { fallBack }) => {
      if (!opcionesPermitidas.includes(ctx.body.toUpperCase())) {
        return fallBack(
          `❌ Lo siento, *${ctx.body}* no es una opción válida. Por favor, intenta de nuevo. *(VINCULAR, INFORMACION, FAQ)*`
        );
      }
    },
