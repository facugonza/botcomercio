...
import flowVender from './flowVender';
...
const opcionesPermitidas = ["LIQUIDACION", "CERTIFICADO", "PLANES", "ASESOR", "VALIDAR", "POS", "DESVINCULAR", "VENDER"];
...
const flowSoyComercio = addKeyword("SoyComercioDeTarjetaDATA", { sensitive: false })
  .addAnswer(
    [
      ...
      "",
      "📞 *Por inconvenientes en Ventas, comunícate al 4292002 o 4292003 en horarios de 09:00hs a 13:00hs y 16:30hs a 20:30hs.*",
    ],
    { capture: true },
    async (ctx, { endFlow, fallBack }) => {
      ...
      if (!opcionesPermitidas.includes(ctx.body.toUpperCase())) {
        return fallBack(
          `❌ Lo siento, *${ctx.body}* no es una opción válida. Por favor, intenta de nuevo. *(LIQUIDACION, CERTIFICADO, ASESOR, VALIDAR, POS, DESVINCULAR)*`
        );
      }
