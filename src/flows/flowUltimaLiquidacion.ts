...
const flowUltimaLiquidacion = addKeyword("liquidacion", { sensitive: false })
  .addAnswer(
    ...
    async (ctx: any, { endFlow, flowDynamic }: any) => {
      databaseLogger.addLog(ctx.from, acciones.DESCARGAR_LIQUIDACION);
      ...
      if (Object.keys(comercio).length > 0) {
        try {
          if (comercio.lastorden) {
            await flowDynamic([
              {
                body: `📄 *Obteniendo tu última Liquidación DATA generada con fecha ${comercio.lastordenfecha}.* ⏳ *Aguarda unos instantes...*`,
              },
            ]);
            ...
            await flowDynamic([
              {
                body: `📑 *Liquidación N° ${comercio.lastorden}*`,
                media: resumenURL,
              },
            ]);
          } else {
          ...
          emailLogger.error("Error obteniendo Liquidación Comercio ", error.stack);
        }
